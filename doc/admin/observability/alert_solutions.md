...
# precise-code-intel-index-queue: frontend_internal_api_error_responses

**Descriptions:**

- _precise-code-intel-index-queue: 5+ frontend-internal API error responses every 5m by route_ (`warning_precise-code-intel-index-queue_frontend_internal_api_error_responses`)
...
- **Kubernetes:**
	- Confirm that `kubectl get pods` shows the `frontend` pods are healthy.
	- Check `kubectl logs precise-code-intel-index-queue` for logs indicate request failures to `frontend` or `frontend-internal`.
- **Docker Compose:**
	- Confirm that `docker ps` shows the `frontend-internal` container is healthy.
	- Check `docker logs precise-code-intel-index-queue` for logs indicating request failures to `frontend` or `frontend-internal`.

# precise-code-intel-index-queue: container_cpu_usage

**Descriptions:**

- _precise-code-intel-index-queue: 99%+ container cpu usage total (1m average) across all cores by instance_ (`warning_precise-code-intel-index-queue_container_cpu_usage`)

**Possible solutions:**

- **Kubernetes:** Consider increasing CPU limits in the the relevant `Deployment.yaml`.
- **Docker Compose:** Consider increasing `cpus:` of the precise-code-intel-index-queue container in `docker-compose.yml`.

# precise-code-intel-index-queue: container_memory_usage

**Descriptions:**

- _precise-code-intel-index-queue: 99%+ container memory usage by instance_ (`warning_precise-code-intel-index-queue_container_memory_usage`)

**Possible solutions:**

- **Kubernetes:** Consider increasing memory limit in relevant `Deployment.yaml`.
- **Docker Compose:** Consider increasing `memory:` of precise-code-intel-index-queue container in `docker-compose.yml`.

# precise-code-intel-index-queue: container_restarts

**Descriptions:**

- _precise-code-intel-index-queue: 1+ container restarts every 5m by instance_ (`warning_precise-code-intel-index-queue_container_restarts`)

**Possible solutions:**

- **Kubernetes:**
	- Determine if the pod was OOM killed using `kubectl describe pod precise-code-intel-index-queue` (look for `OOMKilled: true`) and, if so, consider increasing the memory limit in the relevant `Deployment.yaml`.
	- Check the logs before the container restarted to see if there are `panic:` messages or similar using `kubectl logs -p precise-code-intel-index-queue`.
- **Docker Compose:**
	- Determine if the pod was OOM killed using `docker inspect -f '{{json .State}}' precise-code-intel-index-queue` (look for `"OOMKilled":true`) and, if so, consider increasing the memory limit of the precise-code-intel-index-queue container in `docker-compose.yml`.
	- Check the logs before the container restarted to see if there are `panic:` messages or similar using `docker logs precise-code-intel-index-queue` (note this will include logs from the previous and currently running container).

# precise-code-intel-index-queue: fs_inodes_used

**Descriptions:**

- _precise-code-intel-index-queue: 3e+06+ fs inodes in use by instance_ (`warning_precise-code-intel-index-queue_fs_inodes_used`)
...
# precise-code-intel-index-queue: provisioning_container_cpu_usage_7d

**Descriptions:**

- _precise-code-intel-index-queue: 80%+ or less than 30% container cpu usage total (7d maximum) across all cores by instance_ (`warning_precise-code-intel-index-queue_provisioning_container_cpu_usage_7d`)

**Possible solutions:**

- If usage is high:
	- **Kubernetes:** Consider decreasing CPU limits in the the relevant `Deployment.yaml`.
	- **Docker Compose:** Consider descreasing `cpus:` of the precise-code-intel-index-queue container in `docker-compose.yml`.
- If usage is low, consider decreasing the above values.

# precise-code-intel-index-queue: provisioning_container_memory_usage_7d

**Descriptions:**

- _precise-code-intel-index-queue: 80%+ or less than 30% container memory usage (7d maximum) by instance_ (`warning_precise-code-intel-index-queue_provisioning_container_memory_usage_7d`)

**Possible solutions:**

- If usage is high:
	- **Kubernetes:** Consider decreasing memory limit in relevant `Deployment.yaml`.
	- **Docker Compose:** Consider decreasing `memory:` of precise-code-intel-index-queue container in `docker-compose.yml`.
- If usage is low, consider decreasing the above values.

# precise-code-intel-index-queue: provisioning_container_cpu_usage_5m

**Descriptions:**

- _precise-code-intel-index-queue: 90%+ container cpu usage total (5m maximum) across all cores by instance_ (`warning_precise-code-intel-index-queue_provisioning_container_cpu_usage_5m`)
...
# precise-code-intel-index-queue: provisioning_container_memory_usage_5m

**Descriptions:**

- _precise-code-intel-index-queue: 90%+ container memory usage (5m maximum) by instance_ (`warning_precise-code-intel-index-queue_provisioning_container_memory_usage_5m`)
